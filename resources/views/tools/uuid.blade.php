...
<div class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <div class="hs-dropdown relative [--strategy:absolute] [--adaptive:none]">
                    ...
                    <div class="hs-dropdown-menu transition-[opacity,margin] duration hs-dropdown-open:opacity-100 opacity-0 hidden w-full bg-neutral-800 shadow-md rounded-lg p-2 mt-2 border border-neutral-700 z-50" role="menu" aria-orientation="vertical">
        ...
        <div class="space-y-2" id="ids-list">
            @foreach($ids as $id)
                <div class="flex items-center gap-2 group">
                    <code class="flex-1 py-2 px-3 bg-neutral-900 rounded-lg text-sm text-gray-300 font-mono break-all uuid-item">{{ $id }}</code>
                    <button type="button" class="copy-btn p-2 text-gray-500 hover:text-bulma-primary transition-colors opacity-0 group-hover:opacity-100" data-value="{{ $id }}" title="Copiar">
                        <i class="fa-solid fa-copy"></i>
                    </button>
                </div>
            @endforeach
    ...
    {{-- Toast --}}
    <div id="toast" class="fixed bottom-4 right-4 py-3 px-4 bg-bulma-primary text-neutral-900 rounded-lg shadow-lg font-medium transform translate-y-2 opacity-0 transition-all duration-300 pointer-events-none">
